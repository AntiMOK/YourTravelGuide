...
let currentGuideId = null; // ID of the currently viewed guide from Firestore
let isCurrentGuideLiked = false;
...
async function callGemini(params, isFindingMore, searchKey) {
    findMoreButton.hidden = true;
    findMoreButton.disabled = false;
    findMoreButton.querySelector('span').textContent = 'Find More';
    ...
    if (!isFindingMore) {
        ...
        findMoreButton.querySelector('span').textContent = 'Searching...';
    }
...
function renderResults(data) {
    ...
    Object.keys(groupedByCategory).sort((a,b) => {
        ...
        if (categoryTitle === "Trending Spots") {
            const regularTrending = restaurantsInCategory.filter(r => !r.is_best_of);
            const bestOfTrending = restaurantsInCategory.filter(r => r.is_best_of);

            const restaurantGrid = document.createElement('div');
            restaurantGrid.className = 'restaurant-grid';
            regularTrending.forEach(resto => {
            ...
            if (bestOfTrending.length > 0) {
                ...
                bestOfTrending.forEach((resto, index) => {
...
function createRestaurantCard(resto, isBestOfCard = false, bestOfColor = 'yellow') {
...
function renderGuideCards(container, guides, showDelete) {
    container.innerHTML = '';
...
function updateLikeButtonUI(likeCount, isLiked) {
    if (!currentGuideId) {
...
async function loadAndRenderComments(guideId) {
...
function init() {
    ...
    allNavButtons.forEach(btn => btn.addEventListener('click', () => navigateTo(btn.dataset.view)));
